...
module('Integration | Component | map', function(hooks) {
  ...
  test('it updates the `src` attribute when the argument changes', async function(assert){
    this.setProperties({
       lat: 37.7749,
       lng: -122.4194,
       zoom: 10,
  ...
  test('the default alt attribute can be overwritten', async function(assert){
    await render(hbs`<Map
       @lat="37.7797"
       @lng="-122.4184"
       @zoom="10"
  ...
  test('the src, width, and height attributes cannot be overwritten', async function(assert){
    await render(hbs`<Map
      @lat="37.7797"
      @lng="-122.4184"
      @zoom="10"
